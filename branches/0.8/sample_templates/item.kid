...
<table class="list">
	<thead><tr>
		<th><input name="select" id="select"
			type="checkbox"
			onClick="this.value=check(this.form.select)" /></th>
		<th>Action</th>
	...
	<tfoot><tr>
		<th colspan="${col_cnt}" style="text-align: right;">
			<span id="newlink">
				<a href="?${str_cgi_args}">add item</a>
<a py:if="page != 1" title="First page"
	href="?page=1">&lt;&lt;First</a>
<a py:if="page != 1" title="Previous page"
	href="?page=${page-1}">&lt;Previous</a>
...
<a py:if="max_row > page*show" title="Last page"
	...
	<tbody py:for="row in rows">
		<tr>
			<td><input name="select" id="select" value="${row.id}" type="checkbox" /></td>
			<td>
				<span>
				<a href="?id=${row.id}&amp;page=${page}&amp;${str_cgi_args}"
					>Edit</a></span><span
